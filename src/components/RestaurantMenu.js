      ...
      <div className="p-2" >
        ...
        <ul >
          {restaurantMenu.map((items) => (
            <li data-testid="menu" key={items.card.info.id}>{items.card.info.name} ---
              <button
                data-testid="cartBtnId"
                className="bg-cyan-400 w-20 p-2 m-2 rounded-md hover:bg-gray-500" onClick={() => addFoodItem(items.card.info)}>ADD</button></li>
          )
          )}
        </ul>
